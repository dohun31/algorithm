...
1. `sum과 누적값 변수를 만들어서 0으로 초기화함. stack이 비었을 때 누적값 > 0 이라면 sum += 누적값 \* (2 or 3), 누적값 = 0. stack이 비었을 떄 누적값 == 0 이라면 sum += (2 or 3). stack이 안비었을 떄 누적값 += (2 or 3)`

이렇게 할 경우에 (()[[]])와 같이 () + [[]] 의 경우는 계산 오류가 남.

2. `여는 괄호일 때 sum[-1]이 0이 아니라면 sum에 0을 push해 줌. 괄호 한 쌍이 나오고. sum[-1]이 0이면 sum[-1] += (2 or 3). sum[-1]이 !0이면 sum[-1] \*= (2 or 3).`

이렇게 할 경우에 더 복잡해짐

3. `닫는 괄호일때 stack[-1]이 정수라면 싹다 더하고 한쌍 만들어지면 \*(2 or 3)하고 push. ex) (()) -> ['(', 2] 인 상태에서 )가 들어오면 stack[-1]이 정수이므로 2 \* 2 가 stack에 남게됨. 따라서 마지막에 stack에 정수들만 남는다면 모두 더한 값을 출력, 아니라면 0 출력.`
...
1.  `(i = 4) 먼저 4 일때 indexs는 비었으니까 그냥 indexs에 i를 push해준다. indexs = [4]`
